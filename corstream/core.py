...
Operator = Callable[[AsyncIterable], AsyncIterable]
...
class Stream(Generic[T]):
    ...
    def _add_op(self, op: Operator) -> "Stream":
        """
        Internal helper to register a new transofrmation in the pipeline.
        """
        self._operations.append(op)
        return self
    
    def apply(self) -> AsyncIterable:
        """
    ...
    def log(self, label: Optional[str] = None) -> "Stream[T]":
        ...
        """
        return self._add_op(log_op(label)) # type: ignore
    ...
    def catch(self, handler: Callable[[Exception], Optional[T]]) -> "Stream[T]":
        ...
        """
        return self._add_op(catch_op(handler)) # type: ignore
    ...
    def retry(self, retries: int = 3, delay: float = 0.0) -> "Stream[T]":
        ...
        """
        return self._add_op(retry_op(retries, delay)) # type: ignore
    ...
    def throttle(self, rate: int, per_seconds: float) -> "Stream[T]":
        ...
        """
        return self._add_op(throttle_op(rate, per_seconds)) # type: ignore
    ...
    def map(self, fn: Callable[[T], Union[U, Awaitable[U]]]) -> "Stream[U]":
        ...
        """
        return self._add_op(map_op(fn)) # type: ignore
    
    def map_async(self, fn: Callable[[T], Awaitable[U]], max_concurrent: int = 5) -> "Stream[U]":
        """
        ...
        :return: A new Stream with transformed output.
        """
        return self._add_op(map_async_op(fn, max_concurrent)) # type: ignore
    
    def filter(self, predicate: Callable[[T], Union[bool, Awaitable[bool]]]) -> "Stream[T]":
        """
        ...
        """
        return self._add_op(filter_op(predicate)) # type: ignore
    ...
    def batch(self, size: int = 1) -> "Stream[List[T]]":
        ...
        """
        return self._add_op(batch_op(size)) # type: ignore
    ...
    @classmethod
    def from_iterable(cls, iterable: Union[Iterable[T], AsyncIterable[T]]) -> "Stream[T]":
        """
        ...
        """
        if hasattr(iterable, "__aiter__"): # AsyncIterable
            return cls(iterable)
        
        if hasattr(iterable, "__iter__"): # Iterable
